...
u_int32_t coolingDownTimeInMS = 0;
u_int8_t desiredTemp = 0;
...
void statusHandler(void) {
    ...
    const float internalError = realError(desiredTemp, internalTemp);
    ...
    switch(devStatus) {
        ...
        case WAITING:
            ...
            break;
        case COOLING_DOWN: {
            const float externalError = realError(externalTemp, internalTemp);
...
void loopHandler() {
    ...
    if ((devStatus == ON) && (delayCounters % WARMING_UP_TIME_IN_MS == 0)) {
        statusHandler();
    }
    ...
    if (timerInterruptCounter % 200 == 0) {
        for (int i = 0; i < 10000; i++) delay(1); //esto lo hago para que no envie valores con una frecuencia menor
        readDhtSensorsTask();
...
void readDhtSensorsTask() {
    ...
    if (isnan(internalTemp) || isnan(externalTemp)) {
        if(isnan(internalTemp)) Serial.println("Failed to read from DHT internal sensor!");
        if(isnan(externalTemp)) Serial.println("Failed to read from DHT external sensor!");
    } else {
...
void setCoolingDownTimeInMS(u_int32_t coolingDownTime) {
    coolingDownTimeInMS = coolingDownTime;
}
...
void setDesiredTemp(u_int8_t temp) {
    desiredTemp = temp;
}
